...
class ExchangeRatesFlowTest {
    ...
    fun `Test exchange rates flow emissions`() = runTest {
        val exchangeRates = listOf(
            exchangeRateEntity(currency = "USD", rate = 1.3),
            exchangeRateEntity(currency = "CAD", rate = 1.7),
            exchangeRateEntity(currency = "AUD", rate = 1.9),
        )
        val exchangeRateOverrides = listOf(
            exchangeRateOverrideEntity(currency = "CAD", rate = 1.5)
        )
        exchangeRatesFlow().test {
            awaitItem() // Initial emission, ignored

            // test that the overridden value for CAD is used correctly
            ...
            // test that the default value for CAD is used correctly
